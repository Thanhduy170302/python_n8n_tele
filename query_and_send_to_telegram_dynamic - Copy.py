...
def create_image_from_dataframe(df, output_path):
    try:
        ...
        ax.axis('tight')
        ax.axis('off')
        table = tbl.table(ax, df.values, colLabels=df.columns, loc='center', cellLoc='right', colWidths=[0.2] * len(df.columns))
...
if __name__ == "__main__":
    import sys
    query = sys.argv[1] if len(sys.argv) > 1 else "SELECT [PBH],[thucuoc],[cskh_ob],[cskh_ob_nc],[cskh_sla_lonhon_72],[baohong],[tuvan] ,[tong_ton]  FROM [dbo].[AutocallGiaHanTraTruoc]"
